...
class Reporter:
    ...
    def generate_markdown_report(self) -> str:
        ...
        for probe_id in evaluations_df['probe_id'].unique():
            ...
            # Format latency differences for better readability
            probe_display = probe_data[['target_platform', 'latency_diff_ms', 'quality_verdict', 'quality_confidence']].copy()
            
            # Format latency differences with interpretation
            if 'latency_diff_ms' in probe_display.columns:
                probe_display['latency_interpretation'] = probe_display['latency_diff_ms'].apply(
                    lambda x: f"{x:.1f}ms ({'faster' if x < 0 else 'slower'})" if pd.notnull(x) else "N/A"
                )
                probe_display = probe_display.drop(columns=['latency_diff_ms'])
                probe_display = probe_display.rename(columns={'latency_interpretation': 'latency_vs_baseline'})
            
            report.append(probe_display.to_markdown(index=False))
